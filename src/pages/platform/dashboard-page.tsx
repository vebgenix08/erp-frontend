...
import type { PlatformDashboardActivityItem, PlatformDashboardSummary } from "../../features/platform-dashboard/model/platform-dashboard.types";
  ...
  const load = () => {
    ...
    void getPlatformDashboard()
      .then((result) => {
        ...
        setActivity(result.platformAuditLogs.slice(0, 8).map((item) => ({
          id: item.id,
          label: item.action,
      ...
      .catch((value) => setError(value instanceof Error ? value.message : "Unable to load platform dashboard"));
  };
