...
} from "../../features/platform-feature-flags/api/platform-feature-flags.api";
...
export function PlatformFeaturesPage() {
  ...
  const load = () => {
    setLoading(true);
    void listPlatformFeatureFlags()
      .then(setFlags)
      .catch((value) =>
        setError(value instanceof Error ? value.message : "Unable to load feature catalog"),
      )
  ...
  return (
    <section className="space-y-5">
      ...
      <header className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            Global feature definitions. Tenant access is granted separately through capabilities.
          </p>
        </div>
      ...
      <PlatformFeatureFlagsTable
        ...
        onToggle={(flag) => {
          setBusy(flag.id);
          void updatePlatformFeatureFlag(flag.id, !flag.isEnabled)
            .then((saved) =>
              setFlags((current) => current.map((item) => (item.id === saved.id ? saved : item))),
            )
            .catch((value) =>
              setError(value instanceof Error ? value.message : "Unable to update feature"),
            )
            .finally(() => setBusy(""));
        }}
      />
      <Modal
        open={open}
        title="Add platform feature"
        description="Create a global feature definition without granting it to any tenant."
        onClose={() => setOpen(false)}
      >
        <PlatformFeatureFlagForm
          busy={busy === "create"}
          onCancel={() => setOpen(false)}
          onSubmit={async (input) => {
            setBusy("create");
            try {
              const saved = await createPlatformFeatureFlag(input);
              setFlags((current) => [...current, saved]);
              setOpen(false);
            } finally {
              setBusy("");
            }
