...
import {
  ...
  suspendTenant,
  updateTenant,
} from "../../features/platform-tenants/api/tenants.api";
...
export function PlatformTenantDetailPage() {
  ...
  const mutate = async (operation: () => Promise<TenantRecord>) => {
    setBusy(true);
    setError("");
    try {
      setTenant(await operation());
    } catch (value) {
      setError(value instanceof Error ? value.message : "Tenant update failed");
    } finally {
      setBusy(false);
    }
  };
    ...
    <section className="space-y-5">
      ...
      <header className="flex flex-wrap items-start justify-between gap-3">
        ...
        <Button asChild variant="outline">
          <Link to={`/platform/tenants/${tenant.id}/onboarding`}>Administrator onboarding</Link>
        </Button>
      </header>
      <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_340px]">
        <Card>
          <CardHeader>
            <CardTitle>Institution record</CardTitle>
          </CardHeader>
          <CardContent>
            <TenantForm
              initialValue={tenant}
              submitLabel="Save tenant"
              onSubmit={async (input) => setTenant(await updateTenant(tenant.id, input))}
            />
          </CardContent>
        </Card>
        <Card>
          ...
          <CardContent className="space-y-3">
            <Button
              ...
              onClick={() => void mutate(() => activateTenant(tenant.id))}
            >
              Activate tenant
            </Button>
            <Button
              ...
              onClick={() => void mutate(() => suspendTenant(tenant.id))}
            >
              Suspend tenant
            </Button>
            <Button
              ...
              onClick={() => void mutate(() => deactivateTenant(tenant.id))}
            >
              Deactivate tenant
            </Button>
            <Button
              className="w-full"
              variant="destructive"
              disabled={busy || Boolean(tenant.deletionRequestedAt)}
              onClick={() => {
                const reason = window.prompt("Record the tenant deletion reason");
                if (reason?.trim())
                  void mutate(() => requestTenantDeletion(tenant.id, reason.trim()));
              }}
            >
              Request deletion
            </Button>
            {tenant.deletionRequestedAt ? (
              <p className="text-xs text-rose-600">
                Deletion requested. Purge eligibility follows the configured retention policy.
              </p>
            ) : null}
          </CardContent>
        </Card>
