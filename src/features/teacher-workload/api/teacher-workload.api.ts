...
import type { SaveAvailabilityInput, SaveWorkloadOverrideInput, TeacherWorkloadWorkspace, WorkloadViewMode } from "../model/teacher-workload.types";

const workspaceFields = `
  teacher{id employeeCode fullName department designation staffType primaryCampusId campusIds}
  ...
  timetableEntries{id dayOfWeek startTime endTime periodCount teachingSessionCount campusId campusName programName className sectionName subjectBatchName subjectName componentType state timetableVersionId timetableVersionStatus classSetupPath}
  availabilityExceptions{id campusId dayOfWeek startTime endTime type effectiveFrom effectiveUntil reason}
  responsibilities{id responsibilityType campusId campusName classId className sectionId sectionName effectiveFrom effectiveUntil}
  issues{code severity dayOfWeek startTime endTime classSection subjectName conflictingAssignment reason recommendedAction actionPath}
`;

export async function getTeacherWorkloadWorkspace(input:{teacherId:string;academicYearId:string;viewMode:WorkloadViewMode;weekStartDate:string}) {
  const result = await graphqlClient<{teacherWorkloadWorkspace:TeacherWorkloadWorkspace},{input:typeof input}>(
...
export async function saveTeacherAvailability(input:SaveAvailabilityInput) {
  const result = await graphqlClient<{setTeacherAvailability:{id:string}},{input:SaveAvailabilityInput}>(
...
export async function saveTeacherWorkloadOverride(input:SaveWorkloadOverrideInput) {
  const result = await graphqlClient<{saveTeacherWorkloadPolicy:{id:string}},{input:SaveWorkloadOverrideInput}>(
