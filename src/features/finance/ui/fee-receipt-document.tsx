  ...
  return (
    <div className="relative rounded-xl border border-slate-300 bg-white p-4 shadow-2xs text-[10.5px] leading-tight space-y-3 font-sans w-full max-w-lg mx-auto">
      ...
      <div className="absolute -top-3 left-1/2 -translate-x-1/2">
        <span
          className={`px-3 py-0.5 rounded-full text-[9.5px] font-extrabold tracking-wider text-white shadow-2xs ${badgeBg}`}
        >
          {copyType}
        </span>
      </div>
      ...
      <header className="grid grid-cols-[44px_minmax(0,1fr)_44px] items-center gap-3 border-b border-slate-200 pb-2.5 pt-1 text-center">
        ...
        <div className="shrink-0 flex flex-col items-center">
          ...
          <span className="text-[7.5px] font-extrabold text-slate-500 mt-0.5 uppercase tracking-tighter">
            ESTD. 2020
          </span>
        </div>
        ...
        <div className="flex-1 space-y-0.5">
          ...
          <p className="text-[9px] text-slate-600 font-medium">
            📞 {data.institution.phone} &nbsp; ✉ {data.institution.email} &nbsp; 🌐{" "}
            {data.institution.website}
          </p>
          <p className="text-[8.5px] font-bold text-slate-700 pt-0.5">
      ...
      <div className="text-center">
        <span
          className={`inline-block px-8 py-0.5 text-[11px] font-black tracking-widest text-white rounded-md uppercase ${headerBg}`}
        >
      ...
      <div className="grid grid-cols-1 gap-2 sm:grid-cols-12 items-stretch text-[10px]">
        ...
        <div className="sm:col-span-5 relative rounded-lg border border-slate-200 bg-slate-50/70 p-2 flex flex-col justify-center overflow-hidden">
          <span className="absolute right-1 bottom-0 text-4xl font-black text-slate-200/50 select-none">
            ₹
          </span>
      ...
      <div className="space-y-1">
        <div
          className={`px-2.5 py-0.5 rounded text-white font-extrabold text-[9px] uppercase tracking-wider ${headerBg}`}
        >
          STUDENT INFORMATION
        </div>
        ...
        <table className="w-full border-collapse border border-slate-200 text-[9.5px]">
          <thead>
            <tr className="bg-slate-100 text-slate-700 font-bold border-b border-slate-200 text-[8.5px]">
              ...
              <th className="border-r border-slate-200 py-1 px-1.5 text-right">
                Previous Paid (₹)
              </th>
          ...
          <tbody className="divide-y divide-slate-200 text-slate-800">
            {data.feeItems.map((item) => (
              <tr key={item.slNo} className="hover:bg-slate-50">
                <td className="border-r border-slate-200 py-1 px-1 text-center font-bold">
                  {item.slNo}
                </td>
                <td className="border-r border-slate-200 py-1 px-1.5 font-bold">{item.feeHead}</td>
                <td className="border-r border-slate-200 py-1 px-1.5 text-center text-slate-600 font-medium">
                  {item.feeOrderNo}
                </td>
                <td className="border-r border-slate-200 py-1 px-1.5 text-right font-medium">
                  {fmt(item.totalAmount)}
                </td>
                <td className="border-r border-slate-200 py-1 px-1.5 text-right font-medium">
                  {fmt(item.previousPaid)}
                </td>
                <td className="border-r border-slate-200 py-1 px-1.5 text-right font-extrabold text-slate-900">
                  {fmt(item.paidNow)}
                </td>
                <td className="py-1 px-1.5 text-right font-bold text-slate-700">
                  {fmt(item.balance)}
                </td>
              </tr>
            ))}
      ...
      <div className="grid grid-cols-1 gap-2 sm:grid-cols-12 items-stretch pt-0.5 text-[10px]">
        ...
        <div className="sm:col-span-7 rounded-lg border border-slate-200 bg-white p-2 space-y-1">
          ...
          <div className="space-y-0.5 font-medium">
            <div className="flex justify-between">
              ...
              <span className="font-bold text-slate-900">
                : ₹ {fmt(data.summary.totalOrderAmount)}
              </span>
            </div>
            <div className="flex justify-between">
              <span className="text-slate-500">Total Paid (Before)</span>
              <span className="font-bold text-slate-900">
                : ₹ {fmt(data.summary.totalPaidBefore)}
              </span>
            </div>
            <div className="flex justify-between">
          ...
          <div
            className={`mt-1 flex justify-between px-2.5 py-1 rounded-md text-white font-extrabold ${headerBg}`}
          >
        ...
        <div className="sm:col-span-5 rounded-lg border border-slate-300 bg-slate-50/80 p-2 flex flex-col items-center justify-center text-center">
          ...
          <p
            className={`mt-0.5 text-xl font-black ${isOffice ? "text-emerald-800" : "text-brand-900"}`}
          >
            ₹ {fmt(data.summary.paidNow)}
          </p>
        </div>
      ...
      <footer className="pt-2 flex items-end justify-between border-t border-slate-200 text-[8.5px] text-slate-500">
        <p className="font-semibold text-slate-600 max-w-[240px] leading-tight">
          {data.footerNote ||
            "Note: This is a computer generated receipt and does not require any signature."}
        </p>
...
export function DualFeeReceiptView({ data }: { data: FeeReceiptData }) {
