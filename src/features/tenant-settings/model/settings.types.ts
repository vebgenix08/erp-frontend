...
export interface Campus { id: string; tenantId: string; code: string; name: string; status: "ACTIVE" | "INACTIVE"; address?: string; contactEmail?: string; contactPhone?: string; createdAt: string; updatedAt: string; deactivatedAt?: string; }
export type AcademicUnitType = "SCHOOL" | "PU" | "DEGREE";
export interface CampusAcademicUnit { id:string;tenantId:string;campusId:string;code:string;name:string;type:AcademicUnitType;curriculumOrAffiliationId:string;status:"ACTIVE"|"INACTIVE";createdAt:string;updatedAt:string;deactivatedAt?:string; }
export type CampusAcademicUnitInput = Pick<CampusAcademicUnit,"name"|"type"|"curriculumOrAffiliationId">;
...
export type CampusInput = Pick<Campus, "name"> & Pick<Partial<Campus>, "address" | "contactEmail" | "contactPhone">;
...
export interface TenantAdminDashboardDistribution { key:string;label:string;count:number; }
export interface TenantAdminDashboardTrendPoint { period:string;label:string;value:number; }
export interface TenantAdminDashboardApplication { id:string;applicationNumber?:string;studentName:string;phone?:string;status:string;updatedAt:string; }
export interface TenantAdminDashboardPaymentMethod { method:string;paymentCount:number;amountMinor:number; }
export interface TenantAdminDashboardOutstandingClass { classId:string;className:string;studentCount:number;outstandingMinor:number; }
export interface TenantAdminDashboardSecurityChange { id:string;change:string;subject:string;occurredAt:string;status:string; }
export interface TenantAdminDashboard { activeStudents:number;activeStaff:number;applicationsAwaitingAction:number;admissionsConfirmed:number;collectedTodayMinor:number;outstandingMinor:number;openWorkItems:number;studentsMissingSections:number;studentsMissingFeeOrders:number;enquiriesToday:number;pendingEnquiryFollowUps:number;applicationsSubmittedToday:number;paymentsToday:number;unpaidStudents:number;failedStaffInvites:number;failedFinanceEvents:number;failedAdmissionEvents:number;applicationCount:number;applicationStatusDistribution:TenantAdminDashboardDistribution[];studentClassDistribution:TenantAdminDashboardDistribution[];admissionsTrend:TenantAdminDashboardTrendPoint[];collectionTrend:TenantAdminDashboardTrendPoint[];collectionByPaymentMethod:TenantAdminDashboardPaymentMethod[];topOutstandingClasses:TenantAdminDashboardOutstandingClass[];recentSecurityChanges:TenantAdminDashboardSecurityChange[];recentApplications:TenantAdminDashboardApplication[];recentActivity:TenantAdminDashboardActivity[];generatedAt:string; }
