...
export function NotificationPolicyManagement() {
  ...
  const load = () => {
    ...
    getNotificationPolicy()
      .then(setPolicy)
      .catch((v) =>
        setError(v instanceof Error ? v.message : "Unable to load notification policy"),
      );
  };
      ...
      <header className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <h2 className="text-xl font-bold text-slate-900">
            Notification Policy & Email Templates
          </h2>
          <p className="mt-1 text-sm text-slate-500">
            Choose which business events create email notifications and preview automated email
            templates.
          </p>
        </div>
        ...
        <Card className="p-0 overflow-hidden border-slate-200">
          <CardHeader className="p-4 border-b border-slate-100 bg-slate-50 flex-row items-center justify-between">
            <CardTitle className="text-xs font-bold text-slate-900 flex items-center gap-2">
              <Mail size={16} className="text-brand-600" /> Staff Email Invitation Template Live
              Preview
            </CardTitle>
                ...
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-slate-400 mt-0.5">
                      Amazon SES development channel
                    </p>
                  </div>
                </div>
              ...
              <CardContent className="pt-4 space-y-4">
                ...
                <div className="space-y-1.5">
                  ...
                  <ModernSelect
                    ...
                    onValueChange={(val) => setPolicy({ ...policy, timezone: val })}
                    options={[
            ...
            <Card className="opacity-75 bg-slate-50/50">
              ...
              <CardContent className="pt-4">
                <p className="text-xs text-slate-500 leading-relaxed">
                  SMS can be enabled after a provider, sender identity, delivery webhook and secret
                  are configured in the communication service settings.
                </p>
              </CardContent>
            </Card>
          </div>
            ...
            <CardContent className="p-0">
              <div className="divide-y divide-slate-100">
                {policy.events.map((event) => (
                  <div
                    key={event.event}
                    className="flex flex-wrap items-center justify-between gap-4 p-4 hover:bg-slate-50/50 transition-colors"
                  >
                    <div className="min-w-0 flex-1">
                      <strong className="block text-sm text-slate-800 font-semibold">
                        {event.label}
                      </strong>
                    ...
                    <div className="flex items-center gap-5 shrink-0">
                      ...
                      <label
                        ...
                        title="SMS provider is not configured"
                      >
                        <input
                          type="checkbox"
                          disabled
