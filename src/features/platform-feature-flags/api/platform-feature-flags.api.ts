...
export async function listPlatformFeatureFlags() {
  const result = await graphqlClient<{ platformFeatureFlags: PlatformFeatureFlag[] }>(
    `query PlatformFeatureFlags { platformFeatureFlags { ${fields} } }`,
  );
  return result.platformFeatureFlags;
}
export async function createPlatformFeatureFlag(input: {
  code: string;
  name: string;
  description?: string;
  isEnabled: boolean;
  ...
  const result = await graphqlClient<
    { createPlatformFeatureFlag: PlatformFeatureFlag },
    { input: typeof input }
  >(
...
export async function updatePlatformFeatureFlag(id: string, isEnabled: boolean) {
  const result = await graphqlClient<
    { updatePlatformFeatureFlag: PlatformFeatureFlag },
    { input: { id: string; isEnabled: boolean } }
  >(
    `mutation UpdatePlatformFeatureFlag($input: UpdateFeatureFlagInput!) { updatePlatformFeatureFlag(input: $input) { ${fields} } }`,
    { input: { id, isEnabled } },
  );
