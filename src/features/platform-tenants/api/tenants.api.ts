...
import type { ProvisionTenantInput, ProvisionTenantResult, TenantRecord, TenantUpdateInput } from "../model/tenant.types";
...
export async function listTenants(): Promise<TenantRecord[]> {
  const result = await graphqlClient<{ tenants: { edges: Array<{ node: TenantRecord }> } }, { first: number; after: string | null }>(`
...
export async function provisionTenant(input: ProvisionTenantInput): Promise<ProvisionTenantResult> {
  const result = await graphqlClient<{ provisionTenant: ProvisionTenantResult }, { input: ProvisionTenantInput & { clientRequestId: string } }>(`
...
export async function updateTenant(id: string, input: TenantUpdateInput): Promise<TenantRecord> {
  const result = await graphqlClient<{ updateTenant: TenantRecord }, { input: TenantUpdateInput & { tenantId: string; clientRequestId: string } }>(`
...
export async function deactivateTenant(id: string): Promise<TenantRecord> {
  const result = await graphqlClient<{ deactivateTenant: TenantRecord }, { tenantId: string; clientRequestId: string }>(`
...
async function runTenantLifecycleMutation(field: "activateTenant" | "suspendTenant" | "confirmTenantDeletion", tenantId: string): Promise<TenantRecord> {
  const result = await graphqlClient<Record<typeof field, TenantRecord>, { tenantId: string; clientRequestId: string }>(`
...
export const activateTenant = (tenantId: string) => runTenantLifecycleMutation("activateTenant", tenantId);
export const suspendTenant = (tenantId: string) => runTenantLifecycleMutation("suspendTenant", tenantId);
export const confirmTenantDeletion = (tenantId: string) => runTenantLifecycleMutation("confirmTenantDeletion", tenantId);

export async function requestTenantDeletion(tenantId: string, reason: string): Promise<TenantRecord> {
  const result = await graphqlClient<{ requestTenantDeletion: TenantRecord }, { tenantId: string; reason: string; clientRequestId: string }>(`
