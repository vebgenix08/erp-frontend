...
const staffColumnPreferenceKey = "vebgenix.staff-directory.columns.v2";
...
export function StaffDirectory() {
  ...
  const [pageData, setPageData] = useState<
    Pick<EmployeePage, "page" | "pageSize" | "total" | "totalPages" | "summary">
  >({
    page: 1,
    pageSize: 20,
    total: 0,
    totalPages: 0,
    summary: { total: 0, active: 0, teaching: 0, nonTeaching: 0, loginReady: 0, inviteIssues: 0 },
  });
  ...
  const [selectedColumnIds, setSelectedColumnIds] = useState<StaffColumnId[]>(() =>
    loadColumnPreference(staffColumnPreferenceKey, defaultStaffColumns),
  );
  ...
  useEffect(
    () => setSelected(new Map()),
    [category, loginStatus, search, selectedCampus?.id, status],
  );

  useEffect(() => {
    saveColumnPreference(staffColumnPreferenceKey, selectedColumnIds);
  }, [selectedColumnIds]);
  ...
  const columnDefinitions = useMemo<StaffColumn[]>(() => {
    const base: StaffColumn[] = [
      ...
      { id: "code", label: "Employee Code", width: 18, value: (item) => item.employeeCode },
      {
        ...
        label: "Campus",
        width: 24,
        value: (item) => campusName(item.primaryCampusId),
      },
      ...
      { id: "department", label: "Department", width: 18, value: (item) => item.department ?? "-" },
      {
        ...
        label: "Employment",
        width: 16,
        value: (item) => label(item.employmentType),
      },
      { id: "phone", label: "Phone", width: 16, value: (item) => item.phone ?? "-" },
      {
        ...
        label: "Joining Date",
        width: 16,
        value: (item) => formatDate(item.joiningDate),
      },
  ...
  const exportStaff = async (format: "XLSX" | "PDF") => {
    ...
    try {
      ...
      if (exportScope === "FILTERED" && selectedCampus) {
        const first = await listEmployeePage({
          campusId: selectedCampus.id,
          ...(search ? { search } : {}),
          ...(category ? { staffCategory: category } : {}),
          ...(status ? { status } : {}),
        ...
        for (let nextPage = 2; nextPage <= first.totalPages; nextPage += 1) {
          const next = await listEmployeePage({
            campusId: selectedCampus.id,
            ...(search ? { search } : {}),
            ...(category ? { staffCategory: category } : {}),
            ...(status ? { status } : {}),
  ...
  const renderStaffCell = (employee: Employee, column: StaffColumn) => {
    ...
    return (
      <span
        className={
          column.id === "code" ? "font-mono font-bold text-slate-700" : "font-medium text-slate-700"
        }
      >
        {column.value(employee)}
      </span>
  ...
  return (
    <section className="space-y-5 pb-12 font-sans text-slate-900">
      ...
      <header className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-0.5 text-xs text-slate-500">
            Manage employee records, campus assignments and login readiness for{" "}
            {selectedCampus.name}.
          </p>
        </div>
        <Button
      ...
      <div className="rounded-2xl border border-slate-200/90 bg-white p-3 shadow-2xs space-y-3">
        <div className="flex flex-wrap items-center gap-2.5">
          ...
          >
            ...
            <input
              ...
              placeholder="Search name, code, email, phone, designation..."
              className="h-9 w-full rounded-xl border border-slate-200 bg-slate-50/50 pl-9 pr-3 text-xs font-semibold text-slate-900 outline-none focus:border-brand-500 focus:ring-2 focus:ring-brand-500/20"
            />
          </form>
          ...
          <ModernSelect
            ...
            value={loginStatus}
            onValueChange={(val) => updateQuery({ loginStatus: val })}
            placeholder="All Login States"
            className="w-[140px]"
            options={[
              { label: "All Login States", value: "" },
              { label: "Active", value: "ACTIVE" },
              { label: "Invited", value: "INVITED" },
              { label: "Failed", value: "FAILED" },
              { label: "Not Enabled", value: "NONE" },
          ...
          {(search || category || status || loginStatus) && (
            <Button
              ...
              onClick={() => {
                setDraftSearch("");
                setQuery(
                  new URLSearchParams({ page: "1", pageSize: String(pageSize), status: "ACTIVE" }),
                  { replace: true },
                );
              }}
          ...
          <ModernSelect
            ...
            value={exportScope}
            onValueChange={(val) => updateQuery({ exportScope: val }, false)}
            className="w-[110px]"
            options={[
              { label: "Filtered", value: "FILTERED" },
              { label: "Page", value: "PAGE" },
              { label: `Selected (${selected.size})`, value: "SELECTED", disabled: !selected.size },
            ]}
          />
          ...
          <Button
            ...
            className="h-9 px-3 text-xs font-bold border-slate-200 text-emerald-700 hover:bg-emerald-50 rounded-xl shadow-2xs"
          >
          ...
          <Button
            ...
            className="h-9 px-3 text-xs font-bold border-slate-200 text-rose-700 hover:bg-rose-50 rounded-xl shadow-2xs"
          >
          ...
          <Button
            ...
            className="h-9 text-xs font-bold border-slate-200 rounded-xl shadow-2xs"
          >
            ...
            <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
              <div>
                ...
                <p className="text-[11px] text-slate-500">
                  The same selected columns are used in the table, Excel, and PDF.
                </p>
              </div>
        ...
        <div className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-xs">
                  ...
                  <TableRow
                    ...
                    onKeyDown={(e) => {
                      if (e.key === "Enter" || e.key === " ")
                        navigate(`/admin/staff/${employee.id}`);
                    }}
                  >
                    ...
                    {visibleColumns.map((column) => (
                      <TableCell key={column.id} className="whitespace-nowrap">
                        {renderStaffCell(employee, column)}
                      </TableCell>
                    ))}
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 border-t border-slate-200 bg-slate-50/50 px-4 py-3 text-xs">
            <span className="text-slate-500 font-medium">
              ...
              {Math.min(safePage * pageSize, pageData.total)} of {pageData.total.toLocaleString()}{" "}
              staff members
            </span>
