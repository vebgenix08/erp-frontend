...
import type {
  Employee,
  EmployeeInviteAttempt,
  EmployeeInviteDeliveryEvent,
...
import { TeacherWorkloadPage } from "../../teacher-workload/ui/teacher-workload-page";
...
const tabConfig: Array<{ key: ProfileTab; label: string; icon: LucideIcon }> = [
  ...
  { key: "WORKLOAD", label: "Workload & Timetable", icon: CalendarDays },
...
export function EmployeeProfile() {
      ...
      const offeringMap = new Map(
        campusResults.flatMap((result) =>
          result.offerings.map((offering) => [
            offering.id,
            { ...offering, campusId: result.campusId },
          ]),
        ),
      );
  ...
  const documents = customFields.filter(
    ([key, value]) =>
      key !== profilePhotoKey && value && typeof value === "object" && "fileId" in value,
  );
  ...
  const resend = async () => {
    ...
    try {
      ...
      const [history, providerEvents] = await Promise.all([
        listEmployeeInviteAttempts(employee.id),
        listEmployeeInviteDeliveryEvents(employee.id),
      ]);
      setAttempts(history);
      setDeliveryEvents(providerEvents);
    } catch (value) {
  ...
  const changeActiveState = async () => {
    ...
    if (
      !window.confirm(
        reactivating
          ? "Reactivate this employee and login access?"
          : "Deactivate this employee and disable login access?",
      )
    )
      return;
    setBusy(true);
  ...
  return (
    <section className="space-y-6 pb-12 font-sans text-slate-900">
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="space-y-1">
              ...
              <div className="flex flex-wrap items-center gap-3 text-xs text-slate-500 font-medium">
                ...
                <span>
                  Department:{" "}
                  <strong className="text-slate-800 font-bold">{employee.department || "—"}</strong>
                </span>
              </div>
              ...
              <div className="text-xs text-slate-500 flex items-center gap-1.5 pt-0.5">
                ...
                <span>
                  Primary Campus:{" "}
                  <strong className="text-slate-800 font-bold">
                    {campus(employee.primaryCampusId)}
                  </strong>
                </span>
              </div>
            </div>
      ...
      <nav className="flex gap-1 overflow-x-auto rounded-xl border border-slate-200 bg-white p-1.5 shadow-xs">
        {tabConfig
          .filter((item) => item.key !== "WORKLOAD" || employee.staffCategory === "TEACHING")
          .map((t) => {
      ...
      {tab === "WORKLOAD" ? (
        <TeacherWorkloadPage teacherId={employee.id} embedded />
      ) : (
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
          ...
          <div className="lg:col-span-2 space-y-6">
                ...
                <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-xs space-y-4">
                  ...
                  <dl className="grid grid-cols-1 gap-x-6 gap-y-4 sm:grid-cols-2 text-xs">
                    ...
                    <div>
                      ...
                      <dd className="mt-1 font-bold text-slate-900">
                        {employee.campusIds.map(campus).join(", ")}
                      </dd>
                    </div>
                    <div>
                      <dt className="text-slate-400 font-semibold uppercase tracking-wider text-[10px]">
            ...
            {tab === "MENTORING" && (
              <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-xs space-y-4">
                <h3 className="text-sm font-bold text-slate-900 border-b border-slate-100 pb-3 flex items-center gap-2">
                  <Users className="h-4 w-4 text-amber-600" /> Mentor & Class Teacher
                  Responsibilities
                </h3>
                {assignments.filter((item) =>
                  ["SECTION_INCHARGE", "CLASS_TEACHER", "MENTOR"].includes(item.role),
                ).length ? (
                      ...
                      .map((item) => (
                        ...
                        >
                          ...
                          <p className="text-slate-500 mt-0.5">
                            {item.detail} • {campus(item.campusId)}
                          </p>
                        </div>
                      ))}
                  </div>
                ...
                <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-xs space-y-4">
                  ...
                  <dl className="grid grid-cols-1 gap-x-6 gap-y-4 sm:grid-cols-2 text-xs">
                    <div>
                      ...
                      <dd className="mt-1 font-bold text-slate-900">
                        {employee.email || "Login not enabled"}
                      </dd>
                    </div>
                    <div>
                      <dt className="text-slate-400 font-semibold uppercase tracking-wider text-[10px]">
                      ...
                      <dd className="mt-1">
                        <Badge
                          variant={employee.loginStatus === "ACTIVE" ? "success" : "secondary"}
                        >
                        ...
                        <div key={attempt.id} className="flex items-start gap-3 py-3 text-xs">
                          ...
                          <div className="flex-1 min-w-0">
                            <p className="font-bold text-slate-900">
                              ...
                              <span
                                className={
                                  attempt.status === "SENT" ? "text-emerald-600" : "text-amber-600"
                                }
                              >
                  ...
                  <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-xs space-y-4">
                    ...
                    <div className="divide-y divide-slate-100">
                      {deliveryEvents.map((event) => (
                        ...
                        >
                          <div>
                            <p className="font-bold text-slate-900">{label(event.eventType)}</p>
                            <p className="mt-0.5 text-[11px] text-slate-400">
                              Amazon SES provider event
                            </p>
                          </div>
                  ...
                  {employee.invitedAt && (
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-[11px] text-slate-400 mt-0.5">
                          {new Date(employee.invitedAt).toLocaleString()}
                        </p>
                      </div>
                    </div>
                  )}
